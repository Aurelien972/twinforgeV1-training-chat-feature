/*
  ...
  1. New Tables
    - `data_export_requests` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `request_type` (text) - full_export, partial_export
      - `status` (text) - pending, processing, completed, failed
      - `export_format` (text) - json, csv
      - `included_data` (jsonb) - array of data categories to export
      - `file_url` (text, nullable) - download link when ready
      - `file_size_bytes` (bigint, nullable)
      - `expires_at` (timestamptz) - download link expiration (7 days)
      - `requested_at` (timestamptz)
      - `completed_at` (timestamptz, nullable)
      - `error_message` (text, nullable)

    - `account_deletion_requests` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `status` (text) - pending, scheduled, processing, completed, cancelled
      - `deletion_scheduled_at` (timestamptz) - 30 days from request
      - `delete_all_data` (boolean) - true for complete deletion
      - `anonymize_only` (boolean) - true to keep anonymized stats
      - `reason` (text, nullable) - optional user-provided reason
      - `requested_at` (timestamptz)
      - `completed_at` (timestamptz, nullable)
      - `cancelled_at` (timestamptz, nullable)
      - `cancellation_reason` (text, nullable)

    - `data_privacy_consents` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `consent_type` (text) - terms_of_service, privacy_policy, marketing, analytics, third_party_sharing
      - `consent_given` (boolean)
      - `consent_version` (text) - version of terms/policy accepted
      - `consented_at` (timestamptz)
      - `ip_address` (inet, nullable) - for legal proof
      - `user_agent` (text, nullable) - for legal proof

    - `data_access_log` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `access_type` (text) - export, deletion, anonymization, consent_update
  ...
  2. Extend user_preferences
    - `data_retention_preference` (text) - minimal, standard, extended
...
CREATE TABLE IF NOT EXISTS account_deletion_requests (
  ...
  -- Scheduling
  deletion_scheduled_at timestamptz NOT NULL, -- 30 days from request
  ...
  UNIQUE(user_id) WHERE status IN ('pending', 'scheduled', 'processing')
);
...
CREATE TABLE IF NOT EXISTS data_privacy_consents (
  ...
  consent_version text NOT NULL, -- Version of the document accepted
...
CREATE TABLE IF NOT EXISTS data_access_log (
  ...
  -- Action details (JSON with contextual information)
...
-- Only service role can update export requests (processing)
...
-- data_access_log policies (read-only for users)
...
-- Only service role can write to access log
...
DECLARE
  v_request_id uuid;
BEGIN
  -- Find and update the active deletion request
...
BEGIN
  ...
  -- Also clean up old failed requests (older than 30 days)
  DELETE FROM data_export_requests
  WHERE status = 'failed'
